...
import { StoreAddress, StoreSchema } from '@shoppik/types';
import useStyle from './CreateStoreModal.style';
...
const RegisterStoreForm = ({ toggleForm, store }: RegisterStoreFormProps) => {
	...
	const mutationUpdateStore = trpc.store.updateStoreProfile.useMutation({
		onSuccess() {
			trpcStore.getMyStore.invalidate();
			messageApi.open({ type: 'success', content: 'Update store successfully!' });
			toggleForm?.();
		},
		onError(err) {
			handleToastTrpcError(err.data, messageApi);
	...
	const onSubmit = (values: any) => {
		...
		const storeInfo = {
			...
			avatar: '',
			tags: [{ _id: '64d66dc3f8dcc730bbaecaac', name: 'Twitter', slug: 'twitter' }],
		};
		...
		<>
			...
			<div className={styles.wrapper}>
				<Form layout="vertical" form={form} onFinish={onSubmit}>
					...
					<Flex align="flex-start" gap={theme.marginSM}>
